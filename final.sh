...
updater () {
 ...
 if [ ! -d "$INSTALL_DIR" ]; then
    ...
    wget https://raw.githubusercontent.com/ScrimceNetwork/VPS-MX-FREE/main/Install/zzupdate-master/zzupdate.default-si.conf -O /usr/local/vpsmxup/vpsmxup.default.conf  &> /dev/null
else
	echo ""
fi
...
if [ ! -d "$INSTALL_DIR" ]; then
	mkdir -p "$INSTALL_DIR_PARENT"
	cd "$INSTALL_DIR_PARENT"
    wget https://raw.githubusercontent.com/ScrimceNetwork/VPS-MX-FREE/main/Install/zzupdate-master/zzupdate.default.conf -O /usr/local/vpsmxup/vpsmxup.default.conf  &> /dev/null
else
	echo ""
fi
##PAKETES
